...
%%
%% IMPORTANT — ICAIF CONSTRAINT: the conference allows a HARD 8 pages INCLUDING references
%% and NO appendix / supplementary material. The \appendix section below is therefore part of
%% an EXTENDED (arXiv) version only. For the ICAIF submission, delete everything from
%% "\appendix" onward and confirm the main matter is <= 8 pages. The main body is written to
%% stand alone without the appendix; the appendix only adds detail and full tables.
%%
...
inherited structure, not learned skill (per-metric detail in
Appendix~\ref{app:metrics}). Skill-net is most negative for the weakest base
...
measure (Appendix~\ref{app:cardinality}).
...
%% =====================================================================================
%% EXTENDED-VERSION APPENDIX --- NOT part of the 8-page ICAIF submission.
%% ICAIF '26 forbids appendices/supplementary material and caps the paper at 8 pages
%% including references. Delete everything from the \appendix line onward for ICAIF.
%% =====================================================================================
\appendix

\section{Real-data risk metrics}\label{app:metrics}
On the same out-of-sample window, the gate agent tracks its structural base on every
reported tail and risk metric---expected shortfall (ES$_{99}$), maximum drawdown,
tail ratio, skewness, Sharpe, and Sortino---while both the agent and its base clearly
dominate naive $1/N$ on the downside metrics. The agent-versus-base differences are
within noise, consistent with a mean gate near $0.04$ (mostly staying on the base).
This is the mechanistic reason the tail benefit over $1/N$ is inherited structure
(risk parity / de-risking geometry), not learned skill: the agent does not improve on
the base it starts from. The full per-strategy metric table is emitted by the
experiment as \texttt{metrics\_table.csv} alongside each run.

\section{The scale-and-cardinality confound and its fix}\label{app:cardinality}
The faithfulness comparison has two confounds that a naive implementation would
conflate with unfaithfulness. (i) \emph{Scale.} The signal feature has $35$--$100\times$
the input scale of the return/volatility features. Raw gradient saliency
$\lvert\partial g/\partial o_i\rvert$ is scale-invariant, whereas freeze-ablation and
KernelSHAP are scale-sensitive, so the three tracks live in different units; we place
saliency in gradient$\times$std units to match. (ii) \emph{Cardinality.} The return
block has many features (100 in the tilt observation; 220 in the real gate
observation) versus a single signal feature. Summed group importance therefore
over-weights the large group. We (a) express every method as normalized per-feature
shares and (b) compute the causal group verdict from per-feature ablation, so causal
and attribution share one basis. Normalization does not by itself cancel a group's
feature-count advantage (it is a monotone rescale); the cardinality fix is computing
the causal groups per-feature, and the reported \emph{per-feature Spearman} is the
cardinality-robust statistic. In the synthetic tilt the group premise ``signal is the
top group'' fails purely for this reason (one signal feature vs.\ a 100-feature return
block that out-sums it), even though per feature the signal dominates by $\sim54\times$.

\section{Activity diagnostics and the degenerate guard}\label{app:activity}
Normalized shares sum to one whether the policy has a real mechanism or is responding
to noise, so they cannot reveal an inactive agent. We therefore report, per seed and
aggregated: the mean and standard deviation of the policy's scalar decision over the
rollout, and the raw (un-normalized) $\lvert\text{causal effect}\rvert$ per feature
group. If importances collapse to zero the probe records a logged ``no measurable
mechanism'' null rather than dividing by zero. On the real gate agent all five seeds
are non-degenerate, with gate mean $\approx0.052$ but gate standard deviation
$\approx0.18$: the agent swings its gate substantially around a low mean, which is
what distinguishes ``a real but unprofitable rule'' from ``does nothing.'' The
normalized causal group shares on the real agent are $\approx0.96$ for the returns
block (freeze $0.963$, permute $0.961$), $\approx0.02$ for short volatility, and
$\approx0.014$ for the signal; saliency shares match these closely ($0.962$ returns),
while SHAP shifts a little mass onto the signal ($0.05$), the same mild salience bias
seen in the synthetic tilt.

\section{Observation layout}\label{app:obs}
The gate observation is $[\,\text{window}\times n\ \text{returns}\mid n\ \text{short
vol}\mid 1\ \text{signal}\,]$ (232-dim for the 11-asset real panel with window 20).
The tilt observation adds dual-horizon volatility, momentum, and the base weights:
$[\,\text{returns}\mid\text{short vol}\mid\text{long vol}\mid\text{momentum}\mid
\text{base weights}\mid\text{signal}\,]$ (121-dim for the 5-asset synthetic world).
Under an equal-weight base the base-weights group is constant and therefore inert
(near-zero importance), which is expected rather than a finding.

\section{Reproducibility}\label{app:repro}
PPO with an MLP policy (Stable-Baselines3), 150k training steps, five seeds; window
20; 10\,bps costs. On real data the safe sleeve is resolved by ticker (intermediate
Treasuries, then long Treasuries), never a hardcoded index. Heavy training is run on a
CPU batch cluster with thread caps; a GPU gives no benefit for this small policy, as
the bottleneck is environment stepping. Code and configurations will be released
publicly.
