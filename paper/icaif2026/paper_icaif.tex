...
real robustness sweeps use five agent seeds and ten placebo draws per base. On real
data the safe sleeve is resolved by ticker (intermediate then long Treasuries), never
a hardcoded index. Heavy training runs on a CPU batch cluster with thread caps; a GPU
gives no benefit for this small policy, whose bottleneck is environment stepping. All
reported figures are held out.
...
freeze $0.963$, permute $0.961$; saliency $0.962$; SHAP $0.922$), with the crisis
